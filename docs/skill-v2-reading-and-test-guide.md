...
需要按实现细节学习时，先阅读
[Skill v2 当前实现学习手册](skill-v2-current-implementation-learning.md)；本文件保留为
日常开发时的快速阅读和测试清单。
...
```powershell
go test ./skillv2 -run 'Test(Parse|Prompt|Trace|Recording)' -count=1
go test ./skillcompose -count=1
```

随后运行完整 Skill v2 验收、静态检查和并发检查：
...
每个 `skillv2/testdata/*.json` 都必须是独立、直接根的
