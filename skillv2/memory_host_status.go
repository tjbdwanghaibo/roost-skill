...
func (host *MemoryHost) applyStatusLocked(command StatusCommand) (EffectResult, error) {
	...
	if !ok {
		return EffectResult{}, fmt.Errorf("skillv2: unknown status handle %d", command.Status)
	}
	...
	if duration <= 0 {
		return EffectResult{}, fmt.Errorf("skillv2: status duration must be positive")
	}
...
func (host *MemoryHost) applyAttributeModifierLocked(command AttributeModifierCommand) (EffectResult, error) {
	...
	if command.Operation != "add" && command.Operation != "mul_bp" {
		return EffectResult{}, fmt.Errorf("skillv2: unsupported modifier operation %q", command.Operation)
	}
	if command.DurationTicks <= 0 {
		return EffectResult{}, fmt.Errorf("skillv2: modifier duration must be positive")
	}
	host.nextInstanceSequence++
