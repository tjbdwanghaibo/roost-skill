...
func lowerProgram(artifacts *compileArtifacts) *Program {
	if artifacts == nil || artifacts.ir == nil || !artifacts.lowerReady || artifacts.types.types == nil || artifacts.graph.Index == nil {
		panic("skillv2: lowerProgram called without complete compiler artifacts")
	}
	program := &Program{
		id:                        artifacts.ir.id,
	...
	if len(program.operations) != len(artifacts.identity.Operations) {
		panic(fmt.Sprintf("skillv2: lowering produced %d operations for %d identities", len(program.operations), len(artifacts.identity.Operations)))
	}
	if int(context.nextRandomSite) != len(artifacts.identity.RandomSites) {
		panic(fmt.Sprintf("skillv2: lowering consumed %d random sites for %d identities", context.nextRandomSite, len(artifacts.identity.RandomSites)))
	}
...
func lowerProcessNumericOperation(operation string) processNumericOperation {
	...
	default:
		panic("skillv2: unsupported process numeric operation")
	}
}
...
func lowerProcessPropertyKey(key string) processPropertyKey {
	...
	default:
		panic("skillv2: unsupported process property key")
	}
}
...
func lowerProcessPropertyProcessKind(kind string) processPropertyProcessKind {
	...
	default:
		panic("skillv2: unsupported process property process kind")
	}
}
...
func lowerProcessPropertySlotBinding(binding ProcessPropertySlotBinding) processPropertySlotBindingProgram {
	...
	if stage == 0 || variant == 0 || field == 0 {
		panic("skillv2: unsupported process property slot binding")
	}
...
func (c *loweringContext) lowerFlow(flow flowIR, scope lowerScope) (OperationIndex, bool) {
	...
	if int(index) < len(c.artifacts.identity.Operations) && c.artifacts.identity.Operations[index].Path != header.sourcePath {
		panic(fmt.Sprintf("skillv2: operation identity mismatch at %d", index))
	}
	...
	case *gotoFlowIR:
		...
		if !ok {
			panic("skillv2: unresolved goto reached lower")
		}
	...
	default:
		panic(fmt.Sprintf("skillv2: unsupported flow %T", flow))
	}
	return index, true
}
...
func (c *loweringContext) lowerEffect(header operationHeader, flow *effectFlowIR, scope lowerScope) operation {
	...
	if flow.callbacks != nil || flow.process != nil {
		...
		if flow.process != nil {
			...
			for index, track := range flow.process.numericTracks {
				...
				if !found {
					panic("skillv2: process property was not resolved")
				}
	...
	case *captureSnapshotEffectIR:
		...
		if !found {
			panic("skillv2: temporal profile was not resolved")
		}
	...
	default:
		panic(fmt.Sprintf("skillv2: unsupported effect %T", flow.effect))
	}
}
...
func (c *loweringContext) lowerMotionProgram(value motionIR, scope lowerScope) *motionProgram {
	...
	default:
		panic("skillv2: unsupported motion frame during lowering")
	}
	...
	default:
		panic("skillv2: unsupported motion steering during lowering")
	}
	...
	default:
		panic("skillv2: unsupported motion trajectory during lowering")
	}
	for index, offset := range motion.offsets {
		...
		default:
			panic("skillv2: unsupported motion offset during lowering")
		}
	}
	...
	default:
		panic("skillv2: unsupported motion completion during lowering")
	}
	return result
}
...
func (c *loweringContext) lowerValue(value valueIR, scope lowerScope) programValue {
	...
	default:
		panic(fmt.Sprintf("skillv2: unsupported value %T", value))
	}
}
...
func (c *loweringContext) lowerStateReference(name string) stateReferenceProgram {
	...
	if !found {
		panic("skillv2: unresolved state reference")
	}
