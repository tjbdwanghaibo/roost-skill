...
func (host *MemoryHost) Advance(tick Tick) (WorldRevision, error) {
	...
	defer host.mutex.Unlock()
	if tick < host.tick {
		return host.revision, fmt.Errorf("skillv2: tick moved backwards")
	}
	if tick == host.tick {
...
func (host *MemoryHost) Read(request ReadRequest) (ReadResult, error) {
	...
	default:
		return ReadResult{}, fmt.Errorf("skillv2: unsupported read payload %T", request.Payload)
	}
	return result, nil
}
...
func (host *MemoryHost) PayCosts(payment CostPayment) (CommitReceipt, error) {
	...
	for _, entry := range payment.Entries {
		if entry.Amount < 0 {
			return CommitReceipt{}, fmt.Errorf("skillv2: negative cost")
		}
